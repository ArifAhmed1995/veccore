                 ...
                 typename Backend::Float_v &out)
{
  ...
  Mask_v<Float_v> mask_dev = !(dev < Float_v(-1.e19)|| dev > Float_v(1.e19));

  Float_v num = Float_v(0.15915494309189535f) / gamma;
  Float_v denom = dev * dev + Float_v(1/ 4.0f);
                      ...
                      float *__restrict__ out, size_t kN,  const char *name)
{
  ...
  for (size_t i = index; i < index + 10; i++)
    printf("x = % 8.8f, mean = % 8.8f, gamma = % 8.8f,\
              breit-wigner result = % 8.8f\n", x[i], mean[i],\
              gamma[i], out[i]);
  #endif
                      ...
                      float *__restrict__ out, size_t kN, const char *name)
{
  ...
  size_t index = (size_t)((kN - 100) * drand48());
  for (size_t i = index; i < index + 10; i++)
    printf("x = % 8.8f, mean = % 8.8f,\
    gamma = % 8.8f, result = % 8.8f\n", x[i], mean[i], gamma[i], out[i]);
  #endif
...
int main()
{
    ...
    TestBreitWignerNaiveScalar(x, mean, gamma, out, kN, "Naive Scalar");
      ...
      TestBreitWignerVectorized<backend::UMESimd>(x, mean, gamma, out, kN, "UME::SIMD");
      TestBreitWignerVectorized<backend::UMESimdArray<8>>(x, mean, gamma, out, kN, "UME::SIMD<8>");
      TestBreitWignerVectorized<backend::UMESimdArray<16>>(x, mean, gamma, out, kN, "UME::SIMD<16>");
      TestBreitWignerVectorized<backend::UMESimdArray<32>>(x, mean, gamma, out, kN, "UME::SIMD<32>");
    #endif
